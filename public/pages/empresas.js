...
export async function renderEmpresas(container, params) {
  ...
  function openFormSidebar() {
    const title = editingEmpresa ? 'Editar Empresa' : 'Nueva Empresa';
  ...
  function renderEmptyState() {
    return el('div', { class: 'text-center p-8 bg-gray-50 rounded-lg' }, [
      ...
      el('h3', { class: 'text-xl font-bold mb-2' }, 'No hay empresas registradas'),
      el('p', { class: 'text-gray-600 mb-4' }, 'Comienza agregando tu primera empresa'),
      el('button', {
        class: 'bg-primary text-white px-6 py-2 rounded font-semibold',
        style: { width: 'fit-content' },
        onclick: handleNew
  ...
  function renderDetailsList(items) {
    return el('div', { class: 'space-y-3' }, items.map(empresa =>
      el('details', { class: 'bg-white border rounded-lg' }, [
        el('summary', {
          ...
          el('div', { class: 'flex-1' }, [
            ...
            el('div', { class: 'text-sm text-gray-600' }, `🏦 CUIT: ${formatCUIT(empresa.cuit)}`)
          ]),
